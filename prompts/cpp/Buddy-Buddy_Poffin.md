# C++ Engine Implementation: Buddy-Buddy Poffin
...
- **Effect Builders:** `search_deck_to_bench`
- **Filter Criteria:** `FilterBuilder().supertype("Pokemon").subtype("Basic").max_hp(70).build()`
...
### File: `cpp_engine/src/cards/trainers/items/buddy_buddy_poffin.cpp`
...
TrainerResult execute_buddy_buddy_poffin(GameState& state, const CardInstance& card) {
    ...
    if (!can_play_buddy_buddy_poffin(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Buddy-Buddy Poffin";
        return result;
    }
    ...
    // result.success = effect_result.success;
    // result.requires_resolution = effect_result.requires_resolution;
    result.effect_description = "Buddy-Buddy Poffin effect";
...
void register_buddy_buddy_poffin(LogicRegistry& registry) {
    ...
    auto generator = [](const GameState& state, const CardInstance& card) -> GeneratorResult {
        ...
        if (!result.valid) {
            result.reason = "Cannot play Buddy-Buddy Poffin";
        }
        return result;
    };
...
```cpp
#include "cards/trainers/items/buddy_buddy_poffin.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_buddy_buddy_poffin(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_buddy_buddy_poffin()` with proper validation
...
- [ ] Implement `execute_buddy_buddy_poffin()` using effect builders
  - Use appropriate `effects::` helper
  - Set correct count/min_count
  - Build correct filter criteria
- [ ] Register all card IDs: `sv5-144, sv6-223, sv8pt5-101, me1-167`
