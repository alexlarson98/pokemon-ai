# C++ Engine Implementation: Boss's Orders
...
- **Effect Builders:** `switch_active`
- Requires bench space check in can_play
...
**Pattern:** TARGETED
TARGETED pattern: Generator provides actions with visible targets

**Card IDs:** `me1-114`

### File: `cpp_engine/src/cards/trainers/supporters/bosss_orders.cpp`
 ...
 */
bool can_play_bosss_orders(const GameState& state, PlayerID player_id) {
 ...
 */
TrainerResult execute_bosss_orders(TrainerContext& ctx) {
    TrainerResult result;
    auto& state = ctx.state;
    PlayerID player_id = state.active_player_index;

    if (!can_play_bosss_orders(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Boss's Orders";
        return result;
    }
    ...
    result.success = true;
    result.effect_description = "Boss's Orders effect";
    return result;
}

} // anonymous namespace

void register_bosss_orders(LogicRegistry& registry) {
    // Unified handler using TrainerContext
    auto handler = [](TrainerContext& ctx) -> TrainerResult {
        return execute_bosss_orders(ctx);
    };
    ...
    // Register for all printings using unified handler
    const std::vector<std::string> card_ids = {"me1-114"};
    for (const auto& id : card_ids) {
        registry.register_trainer_handler(id, handler);
        registry.register_generator(id, "trainer", generator);
    }
...
```cpp
#include "cards/trainers/supporters/bosss_orders.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_bosss_orders(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_bosss_orders()` with proper validation
