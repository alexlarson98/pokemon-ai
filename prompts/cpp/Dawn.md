...
- **Filter Criteria:** `FilterBuilder().supertype("Pokemon").subtype("Basic").build()`
...
**Pattern:** SEARCH
SEARCH pattern: Uses resolution stack for hidden zone selection

**Card IDs:** `me2-87, me2-118, me2-129`

### File: `cpp_engine/src/cards/trainers/supporters/dawn.cpp`
...
TrainerResult execute_dawn(TrainerContext& ctx) {
    ...
    if (!can_play_dawn(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Dawn";
        return result;
    }
    ...
    result.success = true;
    result.effect_description = "Dawn effect";
    return result;
}
...
```cpp
#include "cards/trainers/supporters/dawn.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_dawn(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_dawn()` with proper validation
...
- [ ] Implement `execute_dawn()` using effect builders
  - Use appropriate `effects::` helper
  - Set correct count/min_count
  - Build correct filter criteria
- [ ] Register all card IDs: `me2-87, me2-118, me2-129`
