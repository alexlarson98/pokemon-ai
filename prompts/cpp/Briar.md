...
### Card Text
> You can use this card only if your opponent has exactly 2 Prize cards remaining.    During this turn, if your opponent's Active Pokemon is Knocked Out by damage from an attack used by your Tera Pokemon, take 1 more Prize card.
...
### File: `cpp_engine/src/cards/trainers/supporters/briar.cpp`
...
TrainerResult execute_briar(TrainerContext& ctx) {
    ...
    if (!can_play_briar(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Briar";
        return result;
    }
    ...
    result.success = true;
    result.effect_description = "Briar effect";
    return result;
}
...
void register_briar(LogicRegistry& registry) {
    ...
    auto generator = [](const GameState& state, const CardInstance& /*card*/) -> GeneratorResult {
        ...
        if (!result.valid) {
            result.reason = "Cannot play Briar";
        }
...
```cpp
#include "cards/trainers/supporters/briar.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_briar(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_briar()` with proper validation
...
- [ ] Implement `execute_briar()` using effect builders
  - Use appropriate `effects::` helper
  - Set correct count/min_count
  - Build correct filter criteria
- [ ] Register all card IDs: `sv7-132, sv7-163, sv7-171, sv8pt5-100`
