# C++ Engine Implementation: Rare Candy
...
**Note:** The card IDs below (`sv1-191, sv1-256, sv4pt5-89, me1-125, me1-175`) are from standard_cards.json for THIS card.
Use these exact IDs in your registration - do NOT copy IDs from the Nest Ball example above!

### File: `cpp_engine/src/cards/trainers/items/rare_candy.cpp`
 ...
 */
bool can_play_rare_candy(const GameState& state, PlayerID player_id) {
 ...
 */
TrainerResult execute_rare_candy(GameState& state, const CardInstance& card) {
    TrainerResult result;
    PlayerID player_id = state.active_player_index;

    if (!can_play_rare_candy(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Rare Candy";
        return result;
    }
    ...
    // result.success = effect_result.success;
    // result.requires_resolution = effect_result.requires_resolution;
    result.effect_description = "Rare Candy effect";
...
void register_rare_candy(LogicRegistry& registry) {
    auto handler = [](GameState& state, const CardInstance& card) -> TrainerResult {
        return execute_rare_candy(state, card);
    };

    auto generator = [](const GameState& state, const CardInstance& card) -> GeneratorResult {
        GeneratorResult result;
        result.valid = can_play_rare_candy(state, state.active_player_index);
        if (!result.valid) {
            result.reason = "Cannot play Rare Candy";
        }
        return result;
    };

    // Register for all printings
    registry.register_trainer("sv1-191", handler);
    registry.register_generator("sv1-191", "trainer", generator);
    registry.register_trainer("sv1-256", handler);
    registry.register_generator("sv1-256", "trainer", generator);
    registry.register_trainer("sv4pt5-89", handler);
    registry.register_generator("sv4pt5-89", "trainer", generator);
    registry.register_trainer("me1-125", handler);
    registry.register_generator("me1-125", "trainer", generator);
    registry.register_trainer("me1-175", handler);
...
```cpp
#include "cards/trainers/items/rare_candy.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_rare_candy(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_rare_candy()` with proper validation
...
- [ ] Implement `execute_rare_candy()` using effect builders
  - Use appropriate `effects::` helper
  - Set correct count/min_count
  - Build correct filter criteria
- [ ] Register all card IDs: `sv1-191, sv1-256, sv4pt5-89, me1-125, me1-175`
