# C++ Engine Implementation: Night Stretcher
...
- **Effect Builders:** `recover_from_discard`
...
**Pattern:** TARGETED
TARGETED pattern: Generator provides actions with visible targets (discard pile is public)

**Card IDs:** `sv6pt5-61, sv8-251, me1-173`

### File: `cpp_engine/src/cards/trainers/items/night_stretcher.cpp`

```cpp
/**
 * Night Stretcher - Trainer Item (TARGETED Pattern)
 *
 * Card text:
 * "Put a Pokemon or a Basic Energy card from your discard pile into your hand."
 * "You may play any number of Item cards during your turn."
 ...
 */
bool can_play_night_stretcher(const GameState& state, PlayerID player_id) {
...
TrainerResult execute_night_stretcher(TrainerContext& ctx) {
    ...
    if (!can_play_night_stretcher(state, player_id)) {
        result.success = false;
        result.effect_description = "Cannot play Night Stretcher";
        return result;
    }
    ...
    result.success = true;
    result.effect_description = "Night Stretcher effect";
    return result;
}
...
```cpp
#include "cards/trainers/items/night_stretcher.cpp"

void register_all_trainers(LogicRegistry& registry) {
    // ... existing registrations ...
    trainers::register_night_stretcher(registry);
}
```
...
### Core Implementation
- [ ] Implement `can_play_night_stretcher()` with proper validation
...
- [ ] Implement `execute_night_stretcher()` using effect builders
  - Use appropriate `effects::` helper
  - Set correct count/min_count
  - Build correct filter criteria
- [ ] Register all card IDs: `sv6pt5-61, sv8-251, me1-173`
